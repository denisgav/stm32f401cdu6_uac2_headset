...
uint32_t feed_dma(uint8_t *dma_buffer_p,
...
void refresh_i2s_connections(void) {
	...
	uint8_t appbuf_sample_size_in_bytes =
			(current_settings.resolution == 24) ? (4 * 2) : (2 * 2);

	ring_buffer_storage = m_new(uint8_t,
			current_settings.samples_in_i2s_frame_max
					* appbuf_sample_size_in_bytes * 2);
...
void usb_headset_tud_audio_rx_done_pre_read_handler(uint8_t rhport,
	...
	if (current_settings.blink_interval_ms == BLINK_STREAMING) {
		...
		if (current_settings.resolution == 16) {
			...
			machine_i2s_write_stream(&spk_16b_i2s_buffer[0],
					usb_sample_count * 4);
			//}
...
void HAL_I2S_TxHalfCpltCallback(I2S_HandleTypeDef *hi2s) {
	uint32_t transfer_size_in_bytes =
			(current_settings.resolution == 24) ? (4 * 2) : (2 * 2);
	uint32_t sizeof_half_dma_buffer_in_bytes =
			current_settings.samples_in_i2s_frame_max * transfer_size_in_bytes
					/ 2;
...
void HAL_I2S_TxCpltCallback(I2S_HandleTypeDef *hi2s) {
	...
	uint32_t sizeof_half_dma_buffer_in_bytes =
			current_settings.samples_in_i2s_frame_max * transfer_size_in_bytes
					/ 2;
	feed_dma(&spk_i2s_buf[sizeof_half_dma_buffer_in_bytes],
